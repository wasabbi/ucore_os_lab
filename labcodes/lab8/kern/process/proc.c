...
alloc_proc(void) {
	 ...
	 */
     //LAB6 2015080062 : (update LAB5 steps)
    /*
     ...
     */
	 //LAB8:EXERCISE2 2015080062 HINT:need add some code to init fs in proc_struct, ...
        ...
        proc->rq = NULL;
        list_init(&(proc->run_link));   //run_link->prev = run_link, run_link->next = run_link
        proc->time_slice = 0;
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    ret = -E_NO_MEM;
    //LAB4:EXERCISE2 2015080062
    //LAB8:EXERCISE2 2015080062  HINT:how to copy the fs in parent's proc_struct?
    /*
...
load_icode(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 2015080062  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
    ...
    //(5) 设置现在进程的mm, cr3, 重置pgdir (通过 lcr3 MACRO函数)
    mm_count_inc(mm);
    current->mm = mm;
    current->cr3 = PADDR(mm->pgdir);
    lcr3(current->cr3);

    //(6) 设置uargc和uargv在用户态栈
    //   ----------
    //   |kargv[i]|   <--uargv[i]
    //   ----------
    //   | ...... |
